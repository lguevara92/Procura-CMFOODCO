...
const CAMPOS = [
  { name: "fob", label: "FOB / Valor factura" },
...
export function LandedCostForm({ ordenId }: { ordenId: string }) {
  const [state, formAction, pending] = useActionState(calcularLandedCost, null);

  return (
    <form action={formAction} className="flex flex-col gap-3 rounded-lg border border-slate-200 p-4">
